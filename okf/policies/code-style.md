...
(`identifier_name`, `cyclomatic_complexity`, `function_body_length`, `large_tuple`, ...) that
overlap the ecosystem's own [code-structure](code-structure.md) policy rather than this one; a repo
that needs a structural pass runs one as its own scoped initiative, not as a side effect of a
...
("share the link to the documentation instead of rephrasing the information"). This repo has no
per-symbol `docs/reference/` convention of its own yet (`docs/` today holds `index.md` and
`CHANGELOG.md`); the terse-doc-comment rule still applies, it just has nothing generated to check
it against.
...
Why: this repo is small enough (3 files, ~581 Swift lines) to sweep into full compliance in one PR
and go straight to a blocking gate, the same reasoning that made
[OCCTSwiftScripts](https://github.com/SecondMouseAU/OCCTSwiftScripts) the ecosystem's pilot for
this policy. Full research and rationale:
...
Reference implementation: [OCCTSwiftScripts#114](https://github.com/SecondMouseAU/OCCTSwiftScripts/issues/114)/[#115](https://github.com/SecondMouseAU/OCCTSwiftScripts/pull/115).
Filed and tracked as [SwiftPMX#12](https://github.com/SecondMouseAU/SwiftPMX/issues/12).
