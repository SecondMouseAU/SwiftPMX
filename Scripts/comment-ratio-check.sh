...
#
# Part of the ecosystem's code-style policy rollout (issue #12), following the
# pattern OCCTSwiftScripts#114/#115 established as the reference
# implementation; see docs/code-style-policy-proposal-2026-08.md in the
# `ecosystem` repo for the full rationale. A high ratio is sometimes
...
# rationale that belongs in docs/ instead (see the proposal's own
# CurveAdaptors.md finding for what that looks like once it's had years to
# drift): this script surfaces the number, it doesn't judge which case it is.
# Report-only: always exits 0, same as check-docs-existence.py's own
# `--coverage` mode in OCCTSwift/Scripts, and for the same reason: failing
# every file above a threshold on day one, in a codebase nobody has swept
# yet, punishes the report instead of the problem.
#
# Usage:
