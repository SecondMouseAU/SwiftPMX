...
struct PMXTests {
    ...
    /// all index widths 1 byte, no additional UVs, BDEF1 skinning, no textures. `materials` is a list
    /// of per-material surface counts (index-buffer entries, so a multiple of 3) that must sum to
    /// `indices.count`; defaults to one material owning the whole index buffer.
    static func makePMX(vertices: [SIMD3<Float>], indices: [UInt8], version: Float = 2.0,
                        materials: [Int]? = nil) -> Data {
        var d = Data()
    ...
    func toleratesUnreadableMaterials() throws {
        let verts: [SIMD3<Float>] = [.init(0, 0, 0), .init(1, 0, 0), .init(0, 1, 0), .init(1, 1, 0)]
