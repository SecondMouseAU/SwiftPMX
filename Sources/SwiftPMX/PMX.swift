...
public enum PMX {
    ...
    public struct Options: Sendable {
        ...
        /// and reversing triangle winding (so face normals stay outward). Default `true`.
        public var convertToRightHanded: Bool
        /// Merge vertices closer than this (model units) — PMX duplicates positions at UV/material
        /// seams, so welding restores connectivity. `nil` keeps the original PMX indexing. Default `1e-4`.
        public var weldEpsilon: Float?
        /// Drop zero-area triangles — PMX stores edge/point-draw primitives as degenerate faces.
        /// Default `true`.
        public var dropDegenerate: Bool
        /// Uniform scale applied to every coordinate. MMD models are roughly 8 units ≈ 1 m, so pass a
        /// scale if you need real-world units. Default `1` (faithful extraction).
        public var scale: Float

        public init(convertToRightHanded: Bool = true, weldEpsilon: Float? = 1e-4,
    ...
    public static func read(data: Data, options: Options = .default) throws -> Mesh {
        ...
        while t + 2 < faces.count {
            ...
            if options.convertToRightHanded {
                soup.append(positions[a]); soup.append(positions[c]); soup.append(positions[b])
            } else {
    ...
    private static func parse(data: Data, options: Options)
        ...
        try data.withUnsafeBytes { (raw: UnsafeRawBufferPointer) -> ([SIMD3<Float>], [Int], [Int32]?) in
            ...
            guard try cur.u8() == 0x50, try cur.u8() == 0x4D, try cur.u8() == 0x58, try cur.u8() == 0x20
            ...
            for _ in 0..<vertexCount {
                let x = try cur.f32(), y = try cur.f32(), z = try cur.f32()
            ...
            var faces = [Int](); faces.reserveCapacity(indexCount)
    ...
    /// A bounds-checked little-endian sequential reader. Lives only inside `withUnsafeBytes` so the
    /// raw pointer never escapes.
    private struct Cursor {
        let base: UnsafeRawPointer
        let count: Int
        var off = 0

        mutating func ensure(_ n: Int) throws { guard n >= 0, off + n <= count else { throw Error.truncated } }

        mutating func u8() throws -> UInt8 {
            try ensure(1); defer { off += 1 }
            return base.advanced(by: off).loadUnaligned(as: UInt8.self)
        }
        mutating func i32() throws -> Int32 {
            try ensure(4); defer { off += 4 }
        ...
        mutating func skip(_ n: Int) throws { try ensure(n); off += n }

        /// A length-prefixed (Int32) byte string — we only step over it.
        mutating func skipString() throws { try skip(Int(try i32())) }

        /// A PMX variable-width index. Sizes 1/2 are unsigned; size 4 is Int32. Face references are
        /// always non-negative.
        mutating func index(_ size: Int) throws -> Int {
            switch size {
            case 1: return Int(try u8())
            case 2: try ensure(2); defer { off += 2 }; return Int(base.advanced(by: off).loadUnaligned(as: UInt16.self))
        ...
        mutating func skipSkinning(boneIndexSize B: Int) throws {
            let type = try u8()
            switch type {
            case 0: try skip(B)                  // BDEF1
            case 1: try skip(2 * B + 4)          // BDEF2
            case 2: try skip(4 * B + 16)         // BDEF4
            case 3: try skip(2 * B + 4 + 36)     // SDEF (+ c/r0/r1)
            case 4: try skip(4 * B + 16)         // QDEF
